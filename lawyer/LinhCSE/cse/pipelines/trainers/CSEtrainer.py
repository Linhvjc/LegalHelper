...
from cse.encode.CSEencoder import CSEEncoder
from cse.utils.io import load_file
from cse.utils.metric import recall
from cse.utils.regularize import EarlyStopping
from cse.utils.utils import log_embeddings_to_wandb


class CSETrainer(Trainer):
    def __init__(
        self,
        data_args,
        model_args,
        ...
        self.encoder = CSEEncoder()

    def train(self):
        """
        Perform training on the training dataset using the model.
    ...
    def evaluate(self):
        """
        Perform evaluation on the evaluation dataset using the model.

        Returns:
            dict: A dictionary containing the evaluation results, including the total loss and contrastive loss.
    ...
    def evaluate_on_benchmark(
        self,
        top_k_results: list[int] | None = [5, 10, 20],
        log_to_wandb: bool | None = False,
    ):
        """
        ...
        for paths in self.data_args.benchmark_corpus_filenames:
            ...
            logger.info(f'Name benchmark: {name_benchmark}')

            embedding_corpus = None
            ids_corpus = []

            for i in tqdm(range(0, len(corpus), self.train_args.eval_batch_size)):
                documents = []
            ...
            for i in tqdm(range(0, len(benchmark), self.train_args.eval_batch_size)):
                queries = []
    ...
    def get_optimizer(self):
        """
        Initialize and return the optimizer to be used during training.
