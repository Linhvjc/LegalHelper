...
class BM25:
    def __init__(self, corpus):
        ...
        self.avgdl = 0
        # token -> score contribution of token for each doc
        self.token_score = defaultdict(default_token_data)
        self.doc_len = []
        self.idf = {}
    ...
    def _initialize(self, corpus):
        ...
        for token_data in self.token_score.values():
            ...
            token_data['score'] = np.array(
                token_data['score'], dtype=np.float64)
...
class BM25Plus(BM25):
    ...
    def _convert_token_score(self):
        for data in self.token_score.values():
            ...
            score = freq * (self.k1 + 1) / (self.k1 *
                                            (1 - self.b + self.b * doc_len / self.avgdl) + freq)
            data['score'] = score
